...
// Gráfico de pastel: distribución de resultados finales
// Se usa un gráfico de pastel para mostrar la proporción de cada resultado final
function createPieChart(data) {
  d3.select("#pieChart").select("svg").remove();
  const width = 300, height = 300, radius = Math.min(width, height) / 2;

  const total = d3.sum(data, d => d.count);  // Total de ocurrencias
  ...
  arcs.append("text")
      ...
      .attr("font-size", "12px")
      .text(d => `${d.data.label}: ${((d.data.count/total)*100).toFixed(0)}%`);
}
